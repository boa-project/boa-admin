...
{   
    ...
    public function startInst(){
        ...
        \BoA\Core\Security\CryptoUpgrade::runIfNeeded();
    }
    /**
    ...
    /**
     * Check the presence of OpenSSL crypto and option CMDLINE_ACTIVE
     * @static
     * @return bool
     */
    public static function backgroundActionsSupported(){
        return \BoA\Core\Security\Crypto::isAvailable() && ConfService::getCoreConf("CMDLINE_ACTIVE");
    }
    ...
    public function getCurrentRepositoryIdInst()
    {
        ...
        $repoKeys = array_keys($currentRepos);
        return array_shift($repoKeys);
    }
    /**
    ...
    {
        $currentRepos = $this->getLoadedRepositories();
        $repoId = $_SESSION['REPO_ID'] ?? null;
        if($repoId !== null && isSet($currentRepos[$repoId])){
            $repo = $currentRepos[$repoId];
