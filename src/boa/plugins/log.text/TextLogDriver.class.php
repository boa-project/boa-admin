...
class TextLogDriver extends AbstractLogDriver {
    ...
    function initStorage(){
        ...
        if (!file_exists($storageDir)) {
            @mkdir($storageDir, $this->USER_GROUP_RIGHTS);
        }
        $this->open();
    }
    ...
    function init($options) {
        ...
        // Need convert LOG_CHMOD to string in order to get the intval in 8 base.
        $this->USER_GROUP_RIGHTS = isset($this->options['LOG_CHMOD']) ? intval($this->options['LOG_CHMOD'] . '', 8) : 0770;
